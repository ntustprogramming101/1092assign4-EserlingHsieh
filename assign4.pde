...
void setup() {
	...
	for(int j = 1; j < soilHealth[0].length; j++) {
		soilHealth[floor(random(0,8))][j] = 0;
		soilHealth[floor(random(0,8))][j] = 0;
	}
	...
	for(int a = 0; a < 6; a++){
		soldierX[a] = random(0, 8) * SOIL_SIZE;
		soldierY[a] = (floor(random(0, 4)) + a * 4) * SOIL_SIZE;
	}
	...
	for(int a = 0; a < 6; a++){
		cabbageX[a] = floor(random(0,8)) * SOIL_SIZE;
    	cabbageY[a] = floor(random(0,4) + a * 4) * SOIL_SIZE;
	}
	...
	switch (gameState) {
		...
		for(int i = 0; i < soilHealth.length; i++){
			for (int j = 0; j < soilHealth[i].length; j++) {
				...
				int soilIndex = min(ceil(soilHealth[i][j] / 3) - 1, 4);
				int stoneIndex = min(ceil(soilHealth[i][j] / 3) - 6, 4);
				int darkStoneIndex = min(ceil(soilHealth[i][j] / 3) - 11, 4);
		...
		for (int a = 0; a < soldierX.length; a++) {
			// soldier walk
			soldierX[a] = (soldierX[a] + soldierSpeed + 80) % 800 - 80;
			image(soldier, soldierX[a], soldierY[a]);
		}

		for (int a = 0; a < soldierX.length; a++) {
			// soldier bump detection
			if (abs(playerX - soldierX[a]) < 80 && abs(playerY - soldierY[a]) < 80) {
				playerHealth--;
				if(playerHealth == 0){
					gameState = GAME_OVER;
					break;
